...
export default createRoute(async (c) => {
  ...
  // Jika Tamu belum menekan checkout, mereka belum ada sesi pesanan. Arahkan ke login.
  if (!isUserLoggedIn) {
  ...
  try {
    const { results } = await c.env.DB.prepare(
      'SELECT id, status, total_price as grand_total, created_at, order_type, table_id FROM orders WHERE user_id = ? ORDER BY created_at DESC'
    ).bind(userId).all();
    orders = results;
  } catch (error) {
      ...
      <div class="max-w-md mx-auto bg-gray-50 dark:bg-gray-800 min-h-screen relative shadow-2xl overflow-x-hidden transition-colors duration-300">
        ...
        <div class="p-4 space-y-4">
          {orders.length === 0 ? (
            ...
            const typeBadge = order.order_type === 'DINE_IN' ? `Dine In: Meja ${order.table_id}` : (order.order_type === 'TAKEAWAY' ? 'Takeaway' : 'Delivery');
            return (
              ...
              <div class="flex justify-between items-start mb-3">
                <div>
                  <div class="flex items-center gap-2 mb-1">
                     <span class="text-[9px] text-gray-400 dark:text-gray-500 font-bold tracking-wider">{new Date(order.created_at).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                     <span class="text-[8px] bg-orange-100 text-[#ee4d2d] px-1.5 py-0.5 rounded font-black">{typeBadge}</span>
                  </div>
        ...
        <div class="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-md bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.08)] z-[50]">
          <div class="flex justify-around items-center h-[60px] px-2 pb-safe">
            ...
            <a href="/cart" class="flex flex-col items-center gap-1 text-gray-400 dark:text-gray-500 hover:text-[#ee4d2d] transition-colors">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
              <span class="text-[10px] font-semibold">Keranjang</span>
            </a>
            <a href="/orders" class="flex flex-col items-center gap-1 text-[#ee4d2d]">
