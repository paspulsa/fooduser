...
export default createRoute(async (c) => {
  // Ambil daftar meja yang sedang kosong (IDLE)
  ...
  return c.render(
    <div class="bg-gray-100 min-h-screen font-sans">
             ...
             <form id="guestForm" class="space-y-4" onsubmit="event.preventDefault(); submitGuestSession();">
              <div>
                ...
                <div class="relative">
                  ...
                  <input type="text" id="guestName" required placeholder="Cth: Budi" class="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm text-gray-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-[#ee4d2d] transition-all font-medium placeholder-gray-400" />
                </div>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                     <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                  </div>
                </div>
              </div>
      ...
      {/* SCRIPT KLASIK MENGHINDARI VITE ERROR */}
        ...
        async function submitGuestSession() { \
           ...
           try { \
             const res = await fetch("/api/v1/auth/guest-login", { \
               method: "POST", headers: { "Content-Type": "application/json" }, \
               body: JSON.stringify({ guest_name: guestName, table_id: tableId }) \
             }); \
             const data = await res.json(); \
             if (data.success && data.token) { \
               document.cookie = "token=" + data.token + "; path=/; max-age=43200; SameSite=Lax"; \
               showToast("Sesi meja berhasil dibuat!"); \
               setTimeout(() => { window.location.href = "/"; }, 800); \
             } else { \
             ...
             showToast("Gangguan koneksi.", true); \
             btn.disabled = false; btn.innerHTML = originalText; \
           } \
        } \
      ' }} />
    </div>
  , { title: 'Masuk / Pilih Meja - Kedai Pangsit Kembar 88' })
})
