  ...
  return c.render(
    <div class="bg-gray-100 dark:bg-gray-900 min-h-screen font-sans pb-28">
      ...
      <div class="max-w-md mx-auto bg-gray-50 dark:bg-gray-800 min-h-screen relative shadow-2xl overflow-x-hidden transition-colors duration-300">
        ...
        {/* ALAMAT PENGANTARAN / INFO SESI MEJA */}
        <div class="p-4 mt-2">
          <div id="address-section" class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-orange-100 dark:border-gray-700 flex items-start gap-3 relative overflow-hidden">
            ...
            <div class="flex-1">
              <div class="flex justify-between items-center mb-1">
                <h3 class="text-sm font-black text-gray-900 dark:text-white" id="address-header-text">Alamat Pengantaran</h3>
              ...
              <p class="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed font-bold mt-1" id="cart-address-display">
        ...
        <div id="checkout-section">
          ...
          <div class="px-4 mt-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-3 text-xs font-medium text-gray-600 dark:text-gray-400">
              ...
              <div class="flex justify-between border-b border-dashed border-gray-200 dark:border-gray-600 pb-3" id="ongkir-row-container">
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        let isTakeaway = localStorage.getItem('spos_takeaway') === 'true';
        ...
        function adaptUIForSession() {
           if (isDineIn || isTakeaway) {
              ...
              document.getElementById('cart-address-display').innerText = isDineIn ? \`Meja: \${tableName}\` : 'Pesanan Bawa Pulang (Takeaway)';
              document.getElementById('cart-distance-display').innerText = isDineIn ? 'Dine In' : 'Takeaway';
        ...
        function determineOngkir(distanceKm) {
          ...
          const midRange = Number(deliverySettings.mid_range_max) || 3;
          const midPrice = Number(deliverySettings.mid_range_price) || 8000;
          const maxPrice = Number(deliverySettings.max_range_price) || 10000;

          if(distanceKm > maxLimit) return 'OUT';
          if(distanceKm <= freeRange) return 0;
          if(distanceKm <= midRange) return midPrice;
          return maxPrice;
        }
            ...
            navigator.geolocation.getCurrentPosition(
              async (pos) => {
                ...
                try {
                  ...
                  if (resultOngkir === 'OUT') {
                    isOutOfRange = true;
                    document.getElementById('cart-distance-display').innerText = \`Jarak: \${dist.toFixed(1)}KM (Di luar jangkauan)\`;
                    document.getElementById('summary-ongkir').innerText = 'Tidak Tersedia';
                    calculatedOngkir = 0;
                  } else {
                    isOutOfRange = false;
                    document.getElementById('cart-distance-display').innerText = \`Jarak: \${dist.toFixed(1)}KM\`;
                  ...
                  calculateGrandTotal();
                  btn.innerText = 'Deteksi Ulang'; btn.disabled = false;
        ...
        async function applyCoupon() {
           ...
           try {
             ...
             if(data.success) {
               ...
               msgBox.innerText = '✓ Kupon berhasil dipakai!'; msgBox.className = "text-[11px] font-bold mt-2 text-green-500 block";
               calculateGrandTotal();
             } else {
               appliedCouponCode = ''; discountCouponValue = 0;
               msgBox.innerText = '✕ ' + data.message; msgBox.className = "text-[11px] font-bold mt-2 text-red-500 block";
               calculateGrandTotal();
             }
           } catch(e) {}
           finally { document.getElementById('btn-apply-coupon').disabled = false; }
        }
        ...
        function updateQty(index, delta) {
          ...
          appliedCouponCode = ''; discountCouponValue = 0; document.getElementById('coupon-message').classList.add('hidden');
          saveCart();
        }
        ...
        function calculateGrandTotal() {
          ...
          if(isOutOfRange) {
             masterBtn.disabled = true; masterBtn.innerHTML = '<span class="text-sm">Jarak Terlalu Jauh</span>'; return;
          }
          ...
          masterBtn.innerHTML = \`<span class="text-sm">Pesan Sekarang</span><span class="text-sm bg-white/20 px-3 py-1 rounded-lg">\${formatter.format(grandTotal)}</span>\`;
        }

        async function processCheckout() {
           if(isOutOfRange) return showToast('Maaf, lokasi Anda di luar batas pengiriman.', true);
           ...
           let finalOrderType = 'DELIVERY';
           if (isDineIn) finalOrderType = 'DINE_IN';
           else if (isTakeaway) finalOrderType = 'TAKEAWAY';
           ...
           const payload = {
             cart: cart.map(item => ({
               id: item.id, qty: item.qty, additional_price: item.additional_price || 0, note: item.note || ''
             })),
             coupon_code: appliedCouponCode || null,
             address: finalOrderType === 'DELIVERY' ? address : (isDineIn ? tableName : 'Takeaway'),
             notes: notes,
